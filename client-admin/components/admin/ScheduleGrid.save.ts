...
export const saveSchedule = async (params: SaveScheduleParams): Promise<void> => {
  ...
  const subjectNameMapping = new Map<string, string>(); // Mapare: nume introdus -> nume real din DB
  for (const subjectName of subjectsToCreate) {
    try {
      ...
      // Dacă subiectul există deja, createSubject returnează subiectul existent (care poate avea alt nume)
      // Mapăm numele introdus la numele real din baza de date
      subjectNameMapping.set(subjectName.trim().toLowerCase(), newSubject.name);
      // Adaugă subiectul doar dacă nu există deja în listă (evită duplicate)
      if (!updatedSubjects.find(s => s.id === newSubject.id)) {
  ...
  for (const groupColumn of groupsToProcess) {
    ...
    for (const day of DAYS) {
      for (const hour of TIME_SLOTS) {
        ...
        // Folosește numele real din DB dacă există o mapare (pentru subiecte care există deja)
        const actualSubjectName = subjectNameMapping.get(data.subject.trim().toLowerCase()) || data.subject;
        const subjectId = findIdByName(actualSubjectName, updatedSubjects, 'name');
