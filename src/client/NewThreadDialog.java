...
public class NewThreadDialog extends JDialog {
    ...
    public NewThreadDialog(JFrame parent, List<Tag> tags, int userId) {
        super(parent, "Thread Nou", true);
    ...
    private void initUI() {
        ...
        JLabel headerLabel = new JLabel("📝 Thread Nou");
        ...
        titleField.setToolTipText("Introdu titlul thread-ului");
        formPanel.add(titleField, gbc);

        // Tags
        gbc.gridy++;
        JLabel tagLabel = new JLabel("Tag-uri:");
        ...
        tagNames[0] = "-- Selectează tag --";
        for (int i = 0; i < availableTags.size(); i++) {
            tagNames[i + 1] = availableTags.get(i).getName();
        }

        tagComboBox = new JComboBox<>(tagNames);
        tagComboBox.setBackground(new Color(40, 44, 52));
        tagComboBox.setForeground(TEXT_COLOR);
        tagComboBox.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        tagComboBox.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(BORDER_COLOR, 1),
                BorderFactory.createEmptyBorder(5, 10, 5, 10)
        ));
        formPanel.add(tagComboBox, gbc);
        ...
        contentArea.setToolTipText("Introdu conținutul thread-ului");
        ...
        gbc.weighty = 0;
        gbc.fill = GridBagConstraints.HORIZONTAL;
        JLabel charCountLabel = new JLabel("0 caractere");
        ...
        contentArea.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            ...
            private void updateCount() {
                SwingUtilities.invokeLater(() -> {
                    charCountLabel.setText(contentArea.getText().length() + " caractere");
                });
            }
        });
        ...
        JButton createButton = createButton("📝 Creează", SUCCESS_COLOR);
    ...
    private void createThread() {
        ...
        if (content.length() < 10) {
            JOptionPane.showMessageDialog(this,
                    "Conținutul trebuie să aibă cel puțin 10 caractere!",
                    "Eroare",
                    JOptionPane.ERROR_MESSAGE);
            contentArea.requestFocus();
            return;
        ...
        int tagId = 0;
        if (selectedTag != null && !selectedTag.equals("-- Selectează tag --")) {
        ...
        // Here you would send the create request to server
